...
export const aiDlcKnowledgeWorkflow: ProjectData = {
  ...
  troubleshooting: [
    ...
    "정량 지표는 측정 방식을 별도로 정의하고, 공개 페이지에서는 분석 재사용성·문서화 품질·검증 루프 중심으로 효과 설명"
  ],
  results: [
    "동일 유형 이슈 재분석 시 이전 맥락, 확인 순서, 판단 근거를 다시 설명하는 단계 감소",
    "AI가 제시한 원인 후보를 코드·쿼리·테스트 확인 항목으로 전환해 유사 케이스에서 재사용 가능한 검증 흐름 확보",
    "AI 오판과 반증 사례를 지식 베이스에 누적해 다음 분석에서 먼저 확인할 조건으로 재참조",
    "메일/Jira/위키 작성 흐름을 재현 정보, 확인 요청, 다음 액션 중심으로 정리해 외부 커뮤니케이션 초안 품질 안정화"
  ],
  sectionLabels: {
