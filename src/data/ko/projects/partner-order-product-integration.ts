...
export const partnerOrderProductIntegration: ProjectData = {
  ...
  summary:
    "다수의 여행·숙박·레저 제휴사를 주문·상품·검색 흐름에 연결하고, 월 수만 건 규모의 데이터를 안정적으로 처리하도록 API·배치·DB·검색 노출 기준을 표준화한 백엔드 플랫폼 프로젝트",
  tags: ["Java", "Spring", "Oracle", "Batch"],
  featured: true,
  homeOrder: 1,
  overview:
    "복지·여행 커머스 플랫폼에서 외부 제휴사의 주문·취소 데이터, 상품 정보, 검색 노출, 외부 이동 흐름을 내부 API·배치·DB·검색 구조와 연결.\n\n월 수만 건 규모의 주문·상품 데이터가 반복 처리되는 환경에서 API 응답, 배치 보정, DB 저장, 예외 데이터 재처리, 검색 노출 기준을 표준화해 서비스 데이터 신뢰성을 강화.\n\n핵심은 신규 제휴사를 빠르게 온보딩하고, 제휴사별로 다른 연동 조건을 공통 검증 흐름으로 흡수한 백엔드 연동 플랫폼화.",
  problem:
  ...
  contributions: [
    ...
    "월 수만 건 규모로 반복되는 주문·상품 데이터 흐름에서 API 응답, 배치 보정, DB 저장, 예외 데이터 재처리 기준을 수립",
    "상품 파일 수신부터 적재·정규화·후처리·검색 노출까지 이어지는 end-to-end 데이터 검증 흐름을 표준 절차로 정립",
  ...
  results: [
    ...
    "월 수만 건 규모 데이터 처리에서 API·배치·DB·검색 노출 기준을 일관되게 관리할 수 있는 기반 확보",
