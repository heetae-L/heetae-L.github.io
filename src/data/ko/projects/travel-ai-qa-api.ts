...
export const travelAiQaApi: ProjectData = {
  ...
  summary: "Python/Flask 기반 여행 Q&A API를 만들고 Azure OpenAI 호출, 응답 상태 설계, Oracle 저장, Docker/Nomad 운영 검증까지 연결한 AI API 프로젝트",
  tags: ["Python", "Flask", "Azure OpenAI", "Docker"],
  featured: true,
  homeOrder: 2,
  overview:
    "내부 TF에서 TourGPT로 부르던 여행 Q&A API를 운영 가능한 형태로 구축.\n\nFlask API, Azure OpenAI 연동, 대화 이력 저장, Docker/GitLab CI/Nomad 배포 검증까지 수행",
  role: [
  ...
  contributions: [
    ...
    "초기 AI API 운영화 과정에서 민감 설정 분리, dependency 재현성, schema validation 개선 포인트 도출"
  ],
